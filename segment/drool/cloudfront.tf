...
resource "aws_cloudfront_distribution" "this" {
  ...
  aliases             = var.aliases
  comment             = "cf-${var.agency_code}-${var.project_code}-${var.environment}${var.zone}${var.tier}"
  ...
  #tags                = var.tags
  tags = {
    Name = "cf-${var.agency_code}-${var.project_code}-${var.environment}${var.zone}${var.tier}"
  }
  /*
...
*/
  logging_config {
    bucket          = module.cloudfrontlogs_s3.bucket_regional_domain_name
    prefix          = "cloudfront"
  ...
  dynamic "origin" {
    ...
    content {
      ...
      origin_access_control_id = lookup(origin.value, "origin_access_control_id", lookup(lookup(aws_cloudfront_origin_access_control.OAC, lookup(origin.value, "origin_access_control", ""), {}), "id", null))
